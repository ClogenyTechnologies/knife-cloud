          ...
          @columns_with_info.each do |col_info|
            if config[:chef_data] && @chef_data_col_info.include?(col_info)
              ...
              value = if col_info[:nested_values].nil?
                        (col_info[:value_callback].nil? ? (server.send(col_info[:key]).kind_of?(Array) ? server.send(col_info[:key]).join(", ").to_s : server.send(col_info[:key]).to_s) : col_info[:value_callback].call(server.send(col_info[:key])))
                      else
